...
```
agentraid/
├── manifest.json       # 插件配置文件 (MV3, 版本号 2.2.13)
...
2.  **绝对第二优先级**：`快速装填`（只要有且未满用户配置的最大次数限制 `rapidReloadMax`，默认2次，必选它）。
3.  **首次拥有优先**：其他核心技能未获得过的（Lv.0），优先各获得一次：
    *   `自动护盾`、`宝物磁场`、`技能冷却`、`开局推进`
4.  **均已拥有时的二次升级顺序**：用户可通过在侧边栏/Popup中拖拽排序自由定义（由动态配置 `enhancePriority` 决定，默认顺序为：`自动护盾` > `宝物磁场` > `技能冷却` > `开局推进`）。
...
### 4. 侧边栏与 Popup 布局及同步设计
*   **分栏 Tab 切换控制面板**：侧边栏采用 `📊 运行监控` 和 `⚙️ 挂机设置` 双 Tab 选项卡分栏设计。将主开关与当前运行状态卡片常驻在顶部，保障在有限的视口高度内仍能完整展示大容量运行日志与详细战绩统计。
*   **双向同步与持久化**：所有的运行配置（如结算层数、快速装填最大次数、技能优先级拖动排序等）以及用户最后点击的 Tab 页状态（`sidebarActiveTab`）均会通过 `chrome.storage.local` 跨进程进行秒级实时同步并自动持久化，防止页面重载丢失设置。
...
3. **Git 版本提交保障规范**：
   - **每完成一个版本的改动与测试后，必须立刻执行 Git 保存（Git Commit）**。
   - 这能有效备份当前版本的阶段性成果，防止因后续开发过程中的意外中断、意外重载或代码逻辑改乱导致工作成果丢失或项目状态陷入混乱。
